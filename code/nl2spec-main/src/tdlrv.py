...
which is used for raw message trans to trace
'''
...
def run_tdlrv(tdl_file, raw_file):
    ltl2ba_args = ["parsertest", tdl_file,  raw_file]
    ...
    try:
        ...
        eprint("\t~$ export PATH=$PATH:path-to-parsertest-dir\n")
        sys.exit(1)
...
def tdlrv_main(tdl_file, raw_file):

    output, err, exit_code = run_tdlrv(tdl_file, raw_file)
